...
// Configuration
const LEGAL_DOCS_DIR = path.join(__dirname, "../content/legal");
const OUTPUT_DIR = path.join(__dirname, "../public/legal-pdfs");
...
function formatMarkdownContent(content, filename) {
  ...
  // Add document header based on filename
  let title = "Document Légal FATAPLUS";
  let subtitle = "Plateforme Agricole Madagascar";

  switch (filename) {
    case "terms.md":
      title = "Conditions Générales d'Utilisation";
      subtitle = "CGU FATAPLUS - Plateforme Agricole Madagascar";
      break;
    case "privacy.md":
      title = "Politique de Confidentialité";
      subtitle = "Protection des Données - FATAPLUS";
      break;
    case "cookies.md":
      title = "Politique des Cookies";
      subtitle = "Gestion des Cookies - FATAPLUS";
      break;
    case "delete-account.md":
      title = "Procédure de Suppression de Compte";
      subtitle = "Suppression des Données - FATAPLUS";
      break;
    case "fataplus-pitch-deck.md":
      title = "Présentation FATAPLUS";
      subtitle = "Pitch Deck - Plateforme Agricole Madagascar";
      break;
    case "return-policy.md":
      title = "Politique de Retour et Remboursement";
      subtitle = "Conditions de Retour - FATAPLUS";
      break;
    case "eula.md":
      title = "Contrat de Licence Utilisateur Final (EULA)";
      subtitle = "Licence d'Utilisation - FATAPLUS";
      break;
  }
    ...
    await page.pdf({
      ...
      headerTemplate: `
        <div style="font-size: 10px; color: #666; text-align: center; width: 100%; margin: 0 15mm;">
          <span>FATAPLUS - Documents Légaux</span>
        </div>
      `,
      footerTemplate: `
        <div style="font-size: 10px; color: #666; text-align: center; width: 100%; margin: 0 15mm;">
...
// Main function to process all legal documents
async function convertLegalDocsToPdf() {
  console.log("🚀 Conversion des documents légaux FATAPLUS en PDF...\n");

  try {
    // Get all markdown files in the legal directory
    const files = fs
      .readdirSync(LEGAL_DOCS_DIR)
      .filter((file) => file.endsWith(".md"));

    if (files.length === 0) {
      console.log("❌ Aucun fichier markdown trouvé dans", LEGAL_DOCS_DIR);
      return;
    }
    ...
    for (const filename of files) {
      const inputPath = path.join(LEGAL_DOCS_DIR, filename);
...
if (require.main === module) {
  convertLegalDocsToPdf().catch(console.error);
}

module.exports = { convertLegalDocsToPdf };
