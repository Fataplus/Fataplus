...
import { agricultureCalendar, madagascarCrops, madagascarClimate } from '~/shared/data/madagascar-agriculture'
...
export default defineEventHandler(async (event): Promise<CalendarResponse> => {
  ...
  const now = new Date()
  const currentMonthName = requestedMonth || now.toLocaleDateString('fr-FR', { month: 'long' })
  const currentMonth = currentMonthName.charAt(0).toUpperCase() + currentMonthName.slice(1)
  ...
  if (region) {
    ...
    filteredActivities = monthData.activities.filter(activity => {
      if (regionLower.includes('sava') && (activity.includes('vanille') || activity.includes('café'))) return true
      if (regionLower.includes('alaotra') && activity.includes('riz')) return true
      if (regionLower.includes('côte') && (activity.includes('litchi') || activity.includes('girofle'))) return true
 ...
 */
function getClimateInfo(month: string, region?: string): CalendarResponse['climateInfo'] {
