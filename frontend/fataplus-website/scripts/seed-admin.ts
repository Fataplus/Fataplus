...
async function seed() {
  ...
  try {
    ...
    const existingUser = await db
      .select()
      .from(users)
      .where(eq(users.email, 'fenohery.fanomezanirina@gmail.com'))
      .get()

    if (existingUser) {
      console.log('✓ Admin user already exists')
      console.log(`  Email: fenohery.fanomezanirina@gmail.com`)
      console.log(`  Role: ${existingUser.role}`)
      return
    }

    // Hash password using bcrypt
    const passwordHash = await bcrypt.hash('2025Fefe!', 10)
    ...
    await db
      ...
      .values({
        email: 'fenohery.fanomezanirina@gmail.com',
        username: 'fenohery.fanomezanirina',
